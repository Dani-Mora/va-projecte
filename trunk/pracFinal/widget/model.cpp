...
void Model::RenderWithLight() {
    for(unsigned int cara = 0; cara < faces.size(); cara++) {
      Material material = Scene().matlib.material(faces[cara].material);

      // Calcula la normal de la cara que s'esta pintant
      glNormal3f(faces[cara].normal.x, faces[cara].normal.y, faces[cara].normal.z);

      // Calcula la iluminació ambiental del material
      GLfloat f[4] = {material.ka.r, material.ka.g, material.ka.b, material.ka.a};
      glMaterialfv(GL_FRONT, GL_AMBIENT, f);

      // Calcula la iluminació difusa del material
      GLfloat f1[4] = {material.kd.r, material.kd.g, material.kd.b, material.kd.a};
      glMaterialfv(GL_FRONT, GL_DIFFUSE, f1);

      // Calcula la iluminació especular del material
      GLfloat f2[4] = {material.ks.r, material.ks.g, material.ks.b, material.ks.a};
      glMaterialfv(GL_FRONT, GL_SPECULAR, f2);

      // Estableix la brillantor del material
      glMaterialf(GL_FRONT, GL_SHININESS, material.shininess);

      // Realitza el pintat de les cares
      glBegin (GL_POLYGON);
      for (unsigned int vertex = 0; vertex < faces[cara].vertices.size(); vertex++)
      {
        Point p = vertices[faces[cara].vertices[vertex]].coord;
...
void Model::modifName(std::string n)
{
  name = n;
}
  ...
  if (!fp)
  {
    ...
    while (true)
    {
      ...
      // added
        ...
        //std::cout<< " Material " << filename << "  " << &words[1] << endl;
      }
